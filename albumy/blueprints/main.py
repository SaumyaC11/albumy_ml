...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        if not description:
            description = generate_caption_with_azure(f)
        if not tags:
            tags = generate_tags_with_azure(f)
        photo = Photo(
            filename=filename,
...
def generate_caption_with_azure(file):
    analyze_url = config.endpoint + "vision/v3.1/analyze?visualFeatures=Description"
    file.seek(0)
...
def generate_tags_with_azure(file):
    analyze_url = config.endpoint + "vision/v3.1/analyze?visualFeatures=tags"
    headers = {
        "Ocp-Apim-Subscription-Key": config.key,
        "Content-Type": "application/octet-stream",
    }
    ...
    return tag_instances # wrong return type
